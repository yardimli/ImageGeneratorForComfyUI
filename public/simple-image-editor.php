...
<body class="bg-gray-100 flex flex-col items-center justify-center min-h-screen p-4 sm:p-8 font-sans">
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		const resizeCanvas = () => {
			...
			canvas.getObjects().forEach(obj => {
				// This is a simplified scaling. For a more robust solution, you might store
				// original scale and recalculate, but this works for many cases.
				obj.setCoords();
			});
		...
		saveImageBtn.addEventListener('click', async () => {
			...
			// 7. Post the data to the PHP script
			try {
				const formData = new FormData();
				formData.append('imageData', dataURL);

				const response = await fetch('save_image.php', { method: 'POST', body: formData });
				const result = await response.json();

				if (result.success) {
					alert(`Image saved successfully on the server at: ${result.path}`);
				} else {
				...
				saveImageBtn.textContent = 'Save Full-Resolution Image';
			}
		});
	});
</script>
