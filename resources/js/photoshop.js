...
const state = { projects: new Map(), tabs: [], activeProjectId: null, activeLayerId: null, tool: 'move', zoom: 1, selection: null, clipboard: null, moveable: null, panel: 'layers', histories: new Map(), uploadingLayer: null, imageElements: new Map() };
const sizes = [
...
function renderHistory() { if(state.panel!=='history')return; const panel=document.getElementById('historyPanel'),project=activeProject(); if(!project){panel.innerHTML='<div class="ps-panel-empty">Open a project to see its history.</div>';return;} const history=state.histories.get(project.id) || []; panel.innerHTML=`<div class="ps-history-document"><span class="ps-history-icon">▧</span><strong>${escapeHtml(project.name)}</strong></div><ol class="ps-history-list">${history.map((entry,index)=>`<li class="ps-history-entry${index===history.length-1?' current':''}"><span class="ps-history-icon">${entry.icon}</span><span>${escapeHtml(entry.label)}</span></li>`).join('')}</ol>`; }
...
function render() { renderTabs(); renderCanvas(); renderLayers(); if(state.panel==='properties')renderProperties(); if(state.panel==='history')renderHistory(); }
function renderTabs() {
