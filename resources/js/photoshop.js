...
document.getElementById('canvas').addEventListener('pointerdown',event=>{if(state.tool==='zoom'){setZoom(state.zoom*(event.altKey ? .8 : 1.25));return;}if(state.tool!=='marquee'||event.target!==event.currentTarget)return;const canvas=event.currentTarget,rect=canvas.getBoundingClientRect(),start={x:(event.clientX-rect.left)/state.zoom,y:(event.clientY-rect.top)/state.zoom};canvas.setPointerCapture(event.pointerId);const move=e=>{const x=(e.clientX-rect.left)/state.zoom,y=(e.clientY-rect.top)/state.zoom;selectArea({x:Math.max(0,Math.min(start.x,x)),y:Math.max(0,Math.min(start.y,y)),width:Math.min(activeProject().width,Math.abs(x-start.x)),height:Math.min(activeProject().height,Math.abs(y-start.y))});};const up=()=>{canvas.removeEventListener('pointermove',move);canvas.removeEventListener('pointerup',up);};canvas.addEventListener('pointermove',move);canvas.addEventListener('pointerup',up);});
