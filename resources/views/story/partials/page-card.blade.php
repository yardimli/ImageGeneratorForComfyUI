		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="mb-3">
					...
					{{-- START MODIFICATION: Added data-initial-value to track changes --}}
					<textarea name="pages[{{ $index }}][image_prompt]" class="form-control image-prompt-textarea" rows="3" data-initial-value="{{ e($page->image_prompt ?? '') }}">{{ $page->image_prompt ?? '' }}</textarea>
					{{-- END MODIFICATION --}}
					...
					<button type="button" class="btn btn-sm btn-outline-success mt-2 draw-with-ai-btn" data-bs-toggle="modal" data-bs-target="#drawWithAiModal" data-story-page-id="{{ $page->id ?? '' }}">
				...
				{{-- START MODIFICATION: Add position-relative, spinner, and data attributes for modal --}}
				<div class="image-upload-container mb-2 position-relative" style="min-height: 150px; border: 2px dashed #ccc; padding: 10px; text-align: center;">
					<img src="{{ $page->image_path ?? 'https://via.placeholder.com/300' }}"
					     class="page-image-preview"
					     style="max-width: 100%; max-height: 200px; object-fit: contain; {{ $page && $page->image_path ? 'cursor: pointer;' : '' }}"
