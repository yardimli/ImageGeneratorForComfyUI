...
@section('content')
	<div class="container py-4">
		...
		{{-- Quiz Form Section --}}
		<form action="{{ route('stories.quiz.update', $story) }}" method="POST">
			@csrf
			<div class="card">
				<div class="card-header d-flex justify-content-between align-items-center">
					<h5 class="mb-0">Quiz Questions</h5>
					<button type="button" class="btn btn-sm btn-success" id="add-row-btn">Add Question</button>
				</div>
				<div class="card-body">
					<div id="quiz-entries-container">
						@forelse($story->quiz as $index => $entry)
							@include('story.partials.quiz-entry-row', ['index' => $index, 'entry' => $entry])
						@empty
							<p id="no-entries-message" class="text-muted">No quiz questions yet. Use the AI generator above or add questions manually.</p>
						@endforelse
					</div>
				</div>
	...
	{{-- Template for new quiz rows --}}
	<template id="quiz-entry-template">
		@include('story.partials.quiz-entry-row', ['index' => '__INDEX__', 'entry' => null])
	</template>
@endsection

@section('scripts')
	<script src="{{ asset('js/story-quiz.js') }}"></script>
@endsection
